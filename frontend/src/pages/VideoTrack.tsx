...
export default function VideoTrack() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement> | React.DragEvent) => {
    ...
    } catch (err: any) { setTrackError('Upload failed: ' + (err.message || '')) }
  }
      ...
      pollRef.current = window.setInterval(async () => {
        try {
          ...
          if (st.status === 'completed' || st.status === 'error') {
            ...
            if (st.status === 'error') setTrackError(st.error || 'Tracking error')
          }
        } catch {}
      }, 500)
    } catch (err: any) { setTracking(false); setTrackError('Start failed: ' + (err.message || '')) }
  }
  ...
  const rpmOption = {
    ...
    legend: { data: ['RPM(Raw)', 'RPM(Fitted)', 'RPM(Corrected)', 'Angle'], textStyle: { color: '#94a3b8' } },
    grid: { left: 50, right: 50, top: 40, bottom: 30 },
    xAxis: { type: 'value' as const, name: 'Time (s)' },
    yAxis: [{ type: 'value' as const, name: 'RPM' }, { type: 'value' as const, name: 'Angle (°)' }],
    series: [
      { name: 'RPM(Raw)', type: 'line', data: rpmData.time.map((t, i) => [t, rpmData.rpm[i]]), smooth: false, lineStyle: { color: '#3b82f6', width: 1, opacity: 0.4 }, itemStyle: { color: '#3b82f6' }, symbol: 'none' },
      ...(rpmFit ? [
        { name: 'RPM(Fitted)', type: 'line', data: rpmFit.time_s.map((t, i) => [t, rpmFit.fitted_rpm[i]]), smooth: true, lineStyle: { color: '#f59e0b', width: 2 }, itemStyle: { color: '#f59e0b' }, symbol: 'none' },
        { name: 'RPM(Corrected)', type: 'line', data: rpmFit.time_s.map((t, i) => [t, rpmFit.corrected_rpm[i]]), smooth: true, lineStyle: { color: '#10b981', width: 1.5 }, itemStyle: { color: '#10b981' }, symbol: 'none' },
      ] : []),
      ...
      <div className="w-[35%] flex flex-col gap-3 min-w-0">
        <div className="card-glass p-4 flex-shrink-0">
          ...
          <label className="flex flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-600 p-6 cursor-pointer hover:border-primary-400 dark:hover:border-primary-500 transition-colors bg-slate-50 dark:bg-slate-800/50" onDragOver={e => e.preventDefault()} onDrop={handleUpload}>
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">{videoInfo ? videoInfo.filename : 'Drop or click to upload video'}</span>
